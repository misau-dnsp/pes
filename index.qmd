...
df_main_financ <- asset_df %>%
  ...
  select(any_of(columns_to_label),
...
tbl_activities_objectivo <- gen_tbl_activities(df_main, breakdown = "objectivo_pess")
...
max_count <- max(table(df_submission_chron$`_submission_time`))

df_submission_chron %>% 
  ggplot(aes(`_submission_time`)) +
  geom_bar(fill = "#2780e3", width = 0.7) +
  scale_x_date(
    date_breaks = "1 day",
    date_labels = "%b %d"  # abbreviated month + day
  ) +
  scale_y_continuous(
    breaks = seq(0, max_count + 1, by = 1),  # Tick every 1 unit
...
### Resumo do PES `r ano_pes`
#### Sumário por Programa

A tabela abaixo apresenta um resumo das subactividades propostas organizadas por programa. Para cada programa, é indicado o número total de actividades propostas, o custo total estimado, bem como a distribuição das fontes de financiamento previstas — incluindo fundos do Orçamento do Estado (OE), ProSaude e outras fontes externas. A coluna final representa a lacuna de financiamento, ou seja, a diferença entre o custo total estimado e os fundos já identificados. A linha final da tabela agrega os totais de todas as subactividades registadas até à data, permitindo uma visão consolidada. O programa com o maior número de subactividades propostas é o `r tbl_activities_programa$responsavel_programa[which.max(tbl_activities_programa$n[tbl_activities_programa$responsavel_programa != "Total"])]` com `r max(tbl_activities_programa$n[tbl_activities_programa$responsavel_programa != "Total"], na.rm = TRUE)`. O programa com o maior orçamento proposto é o `r tbl_activities_programa$responsavel_programa[which.max(tbl_activities_programa$calc_custo_total[tbl_activities_programa$responsavel_programa != "Total"])]`, com um total valor de `r scales::comma(max(tbl_activities_programa$calc_custo_total[tbl_activities_programa$responsavel_programa != "Total"], na.rm = TRUE), big.mark = ".", decimal.mark = ",")` MT. 

```{r summary-programa-table}
...
Os custos orçamentados para as subaxtividades propostas podem ser directamente associados ao objetivo do PESS ao qual respondem. Como mostra o gráfico abaixo, o objectivo PESS que recebe o maior investimento financeiro é o objectivo intitulado "`r tbl_activities_objectivo$objectivo_pess[which.max(tbl_activities_objectivo$calc_custo_total[tbl_activities_objectivo$objectivo_pess != "Total"])]`" com um custo total alocado de `r scales::comma(max(tbl_activities_objectivo$calc_custo_total[tbl_activities_objectivo$objectivo_pess != "Total"], na.rm = TRUE), big.mark = ".", decimal.mark = ",")` MT.

```{r cost-objective-graph, fig.width=12, fig.height=3.5, fig.align='left', fig.bg="transparent"}

columns_gt_numeric <- tbl_activities_objectivo %>%
  select(where(is.numeric)) %>%
  colnames()

tbl_activities_objectivo %>% 
  filter(objectivo_pess != "Total") %>% 
  mutate(
    objectivo_pess = str_wrap(objectivo_pess, width = 65),  # wrap after n characters
    objectivo_pess = fct_reorder(objectivo_pess, calc_custo_total)
  ) %>% 
  ggplot(aes(calc_custo_total, objectivo_pess)) +
  geom_col(fill = "#2780e3") +
  geom_text(
    aes(label = comma(calc_custo_total)),
    hjust = 0,
    nudge_x = 0.01 * max(tbl_activities_objectivo$calc_custo_total, na.rm = TRUE),
    size = 4
  ) +
  scale_x_continuous(labels = label_number(scale = 1/1000, suffix = " MT"),
                     expand = expansion(mult = c(0, 0.2))  # add 10% padding on the right
  ) +
  labs(
    title = "Custo Total das Subactividades por Objectico PESS",
    x = "Custo (Milhares de MT)",
    y = NULL
  ) +
  theme_minimal(base_size = 13) +
  theme(
...
A tabela abaixo fornece detalhes sobre as fontes de financiamento e as lacunas orçamentárias para as subatividades que se enquadram em cada objetivo do PESS.

```{r cost-objectivo-table}
...
tbl_activities_objectivo %>%
  ...
  tab_style(
    style = cell_text(size = px(13), weight = "bold", align = "left"),
    locations = cells_column_labels(columns = objectivo_pess)
  ) %>%
  ...
  tab_style(
    style = cell_text(size = px(13), align = "left"),
    locations = cells_body(columns = objectivo_pess)
  ) %>%
 
    tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(rows = objectivo_pess == "Total")
  ) %>%

  # Column names
  cols_label(
    objectivo_pess = "Objectivo PESS",
    n = "Qd.",
    calc_custo_total = "Custo",
    financiamento_oe = "OE",
    financiamento_prosaude = "ProSaude",
