...
uid <- assets %>%
  filter(name == "DNSP PES Final 2025.11.02") %>%
  pull(uid) %>% first()
...
tbl_activities_objectivo <- gen_tbl_activities(df_main, breakdown = "prioridade_pess")
...
Os custos orçamentados para as subactividades propostas podem ser directamente associados ao objetivo do PESS ao qual respondem. Como mostra o gráfico abaixo, o objectivo PESS que recebe o maior investimento financeiro é o objectivo intitulado "`r tbl_activities_objectivo$prioridade_pess[which.max(tbl_activities_objectivo$calc_custo_total[tbl_activities_objectivo$prioridade_pess != "Total"])]`" com um custo total alocado de `r scales::comma(max(tbl_activities_objectivo$calc_custo_total[tbl_activities_objectivo$prioridade_pess != "Total"], na.rm = TRUE), big.mark = ".", decimal.mark = ",")` MT.

```{r cost-objective-graph}
#| fig-width: 11
#| fig-height: 6
#| out-width: "100%"
#| fig-align: left
...
tbl_activities_objectivo %>% 
  filter(prioridade_pess != "Total") %>% 
  mutate(
    prioridade_pess = str_wrap(prioridade_pess, width = 65),  # wrap after n characters
    prioridade_pess = fct_reorder(prioridade_pess, calc_custo_total)
  ) %>% 
  ggplot(aes(calc_custo_total, prioridade_pess)) +
...
tbl_activities_objectivo %>%
  ...
  tab_style(
    style = cell_text(size = px(13), weight = "bold", align = "left"),
    locations = cells_column_labels(columns = prioridade_pess)
  ) %>%
  ...
  tab_style(
    style = cell_text(size = px(13), align = "left"),
    locations = cells_body(columns = prioridade_pess)
  ) %>%
 
    tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(rows = prioridade_pess == "Total")
  ) %>%

  # Column names
  cols_label(
    prioridade_pess = "Objectivo PESS",
    n = "Qd.",
    calc_custo_total = "Custo",
    financiamento_oe = "OE",
    financiamento_prosaude = "ProSaude",
